...
filters.message = {
	...
	decode: function(value) {
		...
		if (encodings[encoding] && this.command_id != 4) {
			result.message = encodings[encoding].decode(result.message);
			if (this.data_coding != consts.ENCODING[encodings.detect(result.message)]) {
				this.data_coding = consts.ENCODING[encodings.detect(result.message)];
			}
		}
		return result;
	}
